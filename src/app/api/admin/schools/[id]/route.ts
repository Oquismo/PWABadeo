...
export async function PUT(
  ...
  try {
    ...
    const nameConflict = await prisma.school.findFirst({
      where: { name: name.trim(), id: { not: schoolId } }
    });
    if (nameConflict) {
      return NextResponse.json({ error: 'Ya existe otra escuela con ese nombre' }, { status: 409 });
    }
    ...
    return NextResponse.json({ message: 'Escuela actualizada exitosamente', school: updatedSchool });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Escuela eliminada exitosamente' });
  } catch (error) {
